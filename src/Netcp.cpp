...
namespace NetCp
{
                 ...
                 std::unordered_map<int, sendTask> &mapOfTasks, int id)
  {
    try
    {
      ...
      sendSocket.SendTo(&messageToSend, sizeof(messageToSend), remoteSocket);
      ...
      for (int i = 0; i <= numberOfLoops; ++i)
      {
        ...
        else
        {
          ...
          {
            sendSocket.SendTo(aux_pointer, aux_length, remoteSocket);
          }
          else if (aux_length > 0)
          {
            sendSocket.SendTo(aux_pointer, MAX_PACKAGE_SIZE, remoteSocket);
                    ...
                    std::atomic_bool &abortReceive)
  {
    ...
    try
    {
      ...
      while (!abortReceive)
      {
        receiveSocket.ReceiveFrom(&buffer, MAX_PACKAGE_SIZE);
  ...
  CommandOptions ResolveCommandOption(std::string str)
  {
    if(str == "quit") return CommandOptions::quit;
    if(str == "help") return  CommandOptions::help;
    if(str == "resume") return  CommandOptions::resume;
    if(str == "pause") return  CommandOptions::pause;
    if(str == "abort") return  CommandOptions::abort;
    if(str == "send") return  CommandOptions::send;
    if(str == "receive") return  CommandOptions::receive;
    
    return CommandOptions::Option_Invalid;
  }
  ...
  void AskForInput(std::exception_ptr &eptr, std::atomic_bool &exit)
  {
    ...
    try
    {
      while (!exit)
      {
        ...
        std::cout << "Introduce a command (help if you need it):";
        ...
        CommandOptions option = ResolveCommandOption(userInput);

        switch (option)
        {

        case CommandOptions::quit:
          exit = true;
          break;

        case CommandOptions::resume:
          sstream >> userInput;
          if (userInput.empty())
          {
            break;
        ...
        case CommandOptions::send:
          ...
          else std::cout << "\t" << filename << " doesn't exist.\n";          
          break;

        case CommandOptions::receive:
          sstream >> pathname;
          if (pathname.empty())
          {
            std::cout
        ...
        case CommandOptions::help:
          std::cout << "\nList of available commands:\n "
                       ...
                       "\treceive [PathnameToSaveFile] - starts a process to receive files that are being sent and saves them at PathnameToSaveFile.\n"
